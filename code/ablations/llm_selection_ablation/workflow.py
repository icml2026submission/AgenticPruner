...
def create_pruning_workflow(args):
    workflow = StateGraph(PruningState)

    # Create a shared LLM instance
    shared_llm = get_llm(model_name=args.llm_model)
    ...
    async def analyze_with_state(state):
        ...
        if 'model_name' not in state or not state['model_name']:
            ...
            import re
            match = re.search(r'Prune\s+([\w.]+(?:_[\w.]+)*)\s+', query)
            if match:
    ...
    async def master_with_state(state):
        ...
        if stop_reason == "initialize_phase2":
            state['extended_search_remaining'] = 3
            GLOBAL_STATE['extended_search_remaining'] = 3  # Also update global state
            print(f"[🔧] Initialized extended_search_remaining = 3")
...
async def run_pruning_workflow(model_name: str, query: str, dataset: str = "cifar10", 
                              ...
                              checkpoint_dir: str = "./checkpoints", args=None):
    ...
    try:
        ...
        workflow = create_pruning_workflow(args)
